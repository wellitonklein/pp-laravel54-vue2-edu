...
@section('content')
    <div class="container">
        <div class="row">
            <h3>Ver turma</h3>
            @php
                $linkEdit   = route('admin.class_informations.edit',['class_students' => $class_information->id]);
                $linkDelete = route('admin.class_informations.destroy',['class_students' => $class_information->id]);
            @endphp
            {!! Button::primary(Icon::pencil().' Editar')->asLinkTo($linkEdit) !!}
            {!!
                Button::primary(Icon::remove().' Excluir')->asLinkTo($linkDelete)
                        ->addAttributes([
